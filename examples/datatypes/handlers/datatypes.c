...
__handler__ void datatypes_ph(handler_args_t *args) {
  printf("Packet @ %p (L2: %p) (size %d) flow_id %d\n", args->task->pkt_mem,
  ...
  dtmem->state[coreid].params.userbuf = HOST_ADDR(args) + CORE_COUNT * PAGE_SIZE;
  ...
  // TODO: measure per-packet time, as well as per-message time
