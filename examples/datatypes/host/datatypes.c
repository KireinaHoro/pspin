...
static char doc[] =
    ...
    "receive a message from the sender (without RTS) and writes the received "
    "userbuf to a file.  This file can be later compared with a golden userbuf "
    "output generated by the sender through direct invocation of "
    "MPITypes.\nIn benchmark mode, we tune the dgemm workload and produce "
...
static struct argp_option options[] = {
    ...
    {"rts_port", 'r', "PORT", 0, "UDP port to send RTS to sender"},
...
int run_trial(fpspin_ctx_t *ctx) {
  ...
  for (int i = 0; i < args->num_iterations; ++i) {
    ...
    if (query_once(ctx, args->num_parallel_msgs)) {
  ...
  while (!query_once(ctx, args->num_parallel_msgs))
    ;
  app_data->num_received = 0;
  ret = 1;
