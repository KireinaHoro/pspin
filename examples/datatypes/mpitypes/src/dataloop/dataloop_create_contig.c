					       ...
					       int flag)
{
    ...
    if(count == 1){
    	printf("ktaranov: Perform optimization: ctg(1)[type] ----> [type]\n");
    ...
    /* ktaranov
	if (((old_loop_ptr->kind & DLOOP_KIND_MASK) == DLOOP_KIND_VECTOR)
    )
	{
			printf("ktaranov: Perform optimization: ctg(k)[vec(n b s)[type]] ----> vec(nk b s)[type]\n");
			printf("%ld %ld \n", old_size, old_extent);
