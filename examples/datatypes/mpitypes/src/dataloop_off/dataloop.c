								   ...
								   int size)
{
	...
	PREPEND_PREFIX(Dataloop_update)
	(dest, ptrdiff);
									 ...
									 DLOOP_Offset ptrdiff)
{
	...
	case DLOOP_KIND_VECTOR:
		...
		if (!(dataloop->kind & DLOOP_FINAL_MASK))
		{
			...
			dataloop->loop_params.cm_t.dataloop =
				(DLOOP_Dataloop *)DLOOP_OFFSET_CAST_TO_VOID_PTR(DLOOP_VOID_PTR_CAST_TO_OFFSET(char *) dataloop->loop_params.cm_t.dataloop + ptrdiff);

			PREPEND_PREFIX(Dataloop_update)
			(dataloop->loop_params.cm_t.dataloop, ptrdiff);
		}
		break;
	...
	case DLOOP_KIND_BLOCKINDEXED:
		...
		dataloop->loop_params.bi_t.offset_array =
			(DLOOP_Offset *)DLOOP_OFFSET_CAST_TO_VOID_PTR(DLOOP_VOID_PTR_CAST_TO_OFFSET(char *) dataloop->loop_params.bi_t.offset_array + ptrdiff);
		...
		if (!(dataloop->kind & DLOOP_FINAL_MASK))
		{
			...
			dataloop->loop_params.bi_t.dataloop =
				(DLOOP_Dataloop *)DLOOP_OFFSET_CAST_TO_VOID_PTR(DLOOP_VOID_PTR_CAST_TO_OFFSET(char *) dataloop->loop_params.bi_t.dataloop + ptrdiff);

			PREPEND_PREFIX(Dataloop_update)
			(dataloop->loop_params.bi_t.dataloop, ptrdiff);
		}
		break;
	...
	case DLOOP_KIND_INDEXED:
		...
		dataloop->loop_params.i_t.blocksize_array =
			(DLOOP_Count *)DLOOP_OFFSET_CAST_TO_VOID_PTR(DLOOP_VOID_PTR_CAST_TO_OFFSET(char *) dataloop->loop_params.i_t.blocksize_array + ptrdiff);
		...
		dataloop->loop_params.i_t.offset_array =
			(DLOOP_Offset *)DLOOP_OFFSET_CAST_TO_VOID_PTR(DLOOP_VOID_PTR_CAST_TO_OFFSET(char *) dataloop->loop_params.i_t.offset_array + ptrdiff);
		...
		if (!(dataloop->kind & DLOOP_FINAL_MASK))
		{
			...
			dataloop->loop_params.i_t.dataloop =
				(DLOOP_Dataloop *)DLOOP_OFFSET_CAST_TO_VOID_PTR(DLOOP_VOID_PTR_CAST_TO_OFFSET(char *) dataloop->loop_params.i_t.dataloop + ptrdiff);

			PREPEND_PREFIX(Dataloop_update)
			(dataloop->loop_params.i_t.dataloop, ptrdiff);
		}
		break;
	...
	case DLOOP_KIND_STRUCT:
		...
		dataloop->loop_params.s_t.blocksize_array =
			(DLOOP_Count *)DLOOP_OFFSET_CAST_TO_VOID_PTR(DLOOP_VOID_PTR_CAST_TO_OFFSET(char *) dataloop->loop_params.s_t.blocksize_array + ptrdiff);
		...
		dataloop->loop_params.s_t.offset_array =
			(DLOOP_Offset *)DLOOP_OFFSET_CAST_TO_VOID_PTR(DLOOP_VOID_PTR_CAST_TO_OFFSET(char *) dataloop->loop_params.s_t.offset_array + ptrdiff);
		...
		dataloop->loop_params.s_t.dataloop_array =
			(DLOOP_Dataloop **)DLOOP_OFFSET_CAST_TO_VOID_PTR(DLOOP_VOID_PTR_CAST_TO_OFFSET(char *) dataloop->loop_params.s_t.dataloop_array + ptrdiff);
		...
		for (i = 0; i < dataloop->loop_params.s_t.count; i++)
		{
			...
			looparray[i] = (DLOOP_Dataloop *)DLOOP_OFFSET_CAST_TO_VOID_PTR(DLOOP_VOID_PTR_CAST_TO_OFFSET(char *) looparray[i] + ptrdiff);
		}
...
void PREPEND_PREFIX(Dataloop_alloc)(int kind,
									...
									int *new_loop_sz_p)
{
	PREPEND_PREFIX(Dataloop_alloc_and_copy)
	(kind,
	 count,
	 NULL,
	 0,
											 ...
											 int *new_loop_sz_p)
{
	...
	if (old_loop != NULL)
	{
		PREPEND_PREFIX(Dataloop_copy)
		(pos, old_loop, old_loop_sz);
	}
										   ...
										   int *new_loop_sz_p)
{
	...
	/* set all the pointers in the new dataloop structure */
	new_loop->loop_params.s_t.dataloop_array = (DLOOP_Dataloop **)(((char *)new_loop) + loop_sz);
	new_loop->loop_params.s_t.blocksize_array = (DLOOP_Count *)(((char *)new_loop) + loop_sz + ptr_sz);
	new_loop->loop_params.s_t.offset_array = (DLOOP_Offset *)(((char *)new_loop) + loop_sz + ptr_sz + blk_sz);
	new_loop->loop_params.s_t.el_extent_array = (DLOOP_Offset *)(((char *)new_loop) + loop_sz + ptr_sz + blk_sz + off_sz);

	*old_loop_p = (DLOOP_Dataloop *)(((char *)new_loop) + loop_sz + ptr_sz + blk_sz + off_sz + extent_sz);
	*new_loop_p = new_loop;
	*new_loop_sz_p = new_loop_sz;
									...
									int depth)
{
	...
	case DLOOP_KIND_CONTIG:
		...
		if (!(dataloop->kind & DLOOP_FINAL_MASK))
			PREPEND_PREFIX(Dataloop_print)
			(dataloop->loop_params.c_t.dataloop, depth + 1);
		break;
	case DLOOP_KIND_VECTOR:
		...
		if (!(dataloop->kind & DLOOP_FINAL_MASK))
			PREPEND_PREFIX(Dataloop_print)
			(dataloop->loop_params.v_t.dataloop, depth + 1);
		break;
	case DLOOP_KIND_BLOCKINDEXED:
		...
		if (!(dataloop->kind & DLOOP_FINAL_MASK))
			PREPEND_PREFIX(Dataloop_print)
			(dataloop->loop_params.bi_t.dataloop, depth + 1);
		break;
	case DLOOP_KIND_INDEXED:
		...
		if (!(dataloop->kind & DLOOP_FINAL_MASK))
			PREPEND_PREFIX(Dataloop_print)
			(dataloop->loop_params.i_t.dataloop, depth + 1);
		break;
	case DLOOP_KIND_STRUCT:
		...
		{
			PREPEND_PREFIX(Dataloop_print)
			(dataloop->loop_params.s_t.dataloop_array[i], depth + 1);
		}
		break;
	default:
		DLOOP_Assert(0);
