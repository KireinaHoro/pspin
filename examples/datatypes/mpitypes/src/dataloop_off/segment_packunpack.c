									   ...
									   void *v_paramp)
{
	...
	if (paramp->direction == DLOOP_M2M_TO_USERBUF)
	{
		//printf("target: %p; off: %lu; source: %p; size: %lu\n", paramp->userbuf, rel_off, paramp->streambuf, size);
...
#define OLDCODE 0
...
#else
	{
		...
		//printf("MPITYPES blocklen: %u; el_size: %u\n", blocklen, el_size);
			...
			if (blocklen == 1)
			{
				...
				else
				{
					while (blocks_left--)
					{
						...
						{
							//printf("MPITYPES el_size 4 copy\n");
								  ...
								  void *streambuf)
{
	...
	PREPEND_PREFIX(Segment_manipulate)
	(segp, first, lastp,
